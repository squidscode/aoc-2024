...
type game = {width: int; height: int; guard: guard; obstructions: coord list};;
...
let part_one (game: game): int =
  match game with 
  { width; height; guard = { position ; direction } ; obstructions } -> begin
    let rec get_traveled_coords (acc: coord list) (gp: coord) (gd: direction): coord list = 
      let diff = get_diff gd in
      let new_coord = apply_diff gp diff in
      begin match (List.mem new_coord obstructions, out_of_bounds width height new_coord) with 
    ...
    in
    get_traveled_coords [] position direction
      |> List.length
  end
;;
...
let part_two (game: game): int =
  match game with 
  { width; height; guard = { position ; direction } ; obstructions } -> begin
    let rec get_traveled_coords 
      (acc: (coord * direction) list) (gp: coord) (gd: direction) 
      ...
      | (_, true, false) -> 
        get_traveled_coords acc gp (rotate_90_degrees_clockwise gd) obstructions
      | (_, false, false) -> 
    ...
    tc
      ...
      |> List.mapi (fun i s -> 
          if (i mod 100) = 0 then (Format.printf "\n#%d done" i)
          else if (i mod 10) = 0 then (Format.printf ".") ;
          Format.print_flush () ;
...
let parse_file (file_content: string): game =
  let lines: string list = file_content
    |> String.split_on_char '\n'
    |> List.filter (fun s -> not (String.equal "" s)) in
  let coords: ((int * int) * char) list = lines
    |> List.map String.to_seqi
    |> List.mapi (fun y l -> Seq.map (fun (x, c) -> ((x,y), c)) l |> List.of_seq)
    |> List.flatten in
  let guard: guard = coords
    |> List.filter (fun (_, c) -> List.mem c ['^'; '>'; 'v'; '<'])
    |> List.hd 
    |> (fun ((x,y), c) -> {position = {x = x; y = y}; direction = match c with
      | '^' -> Up
      | '>' -> Left
      | 'v' -> Down
      | '<' -> Right
  ...
  let obstructions: coord list = coords
    |> List.filter (fun (_, c) -> c = '#')
    |> List.map (fun ((x,y), _) -> {x=x; y=y}) in
  { 
