...
let tlbr_diagonal (l: string list): string list =
  ...
  let rec diagonal acc x y =
    if x < nx && y < ny then begin
      diagonal (acc ^ String.make 1 (String.get (List.nth l y) x)) (x + 1) (y + 1)
    end else begin 
      acc
    end
  in
  let left_side = List.mapi (fun i _ -> diagonal "" 0 i) l in
  let top_side = List.of_seq 
    (Seq.mapi (fun i _ -> diagonal "" i 0) (String.to_seq (List.hd l))) in
...
let bltr_diagonal (l: string list): string list =
  ...
  let bottom_side = List.of_seq 
    (Seq.mapi (fun i _ -> diagonal "" i (ny-1)) (String.to_seq (List.hd l))) in
...
let part_two (l: string list): int =
  ...
  let a_coords = coords
    ...
    |> List.filter (fun ((x, y), _) -> 1 <= x && x <= nx - 2 && 1 <= y && y <= ny - 2)
  in
  let xmas's = a_coords
