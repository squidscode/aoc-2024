...
let adj_list (edges : (int * int) list) : (int * int list) list =
  let rec add_rule_to_adj adj (l, r) =
    ...
    | [] -> [ (l, [ r ]) ]
  in
  edges |> List.fold_left add_rule_to_adj []
...
let topological_sort (edges : (int * int) list) : int list =
  ...
  let in_count =
    ...
    |> List.fold_left (fun l i ->
           if Option.is_some (List.assoc_opt i l) then l else (i, 0) :: l)
  ...
  *)
  let rec help (acc : int list) (in_count : int IntMap.t) : int list =
    let zeros : int list =
      ...
      |> List.of_seq
    in
    let non_zeros : int IntMap.t =
      in_count |> IntMap.filter (fun _ n -> n != 0)
    in
    (*
    ...
    *)
    let new_in_count : int IntMap.t =
      zeros
      |> List.fold_left
           (fun mp i ->
             List.assoc_opt i adjs |> Option.value ~default:[]
             |> List.fold_left
                  (fun mp node ->
                    IntMap.update node (Option.map (fun n -> n - 1)) mp)
                  mp)
           non_zeros
    in
    if List.length zeros = 0 then acc else help (acc @ zeros) new_in_count
  in
  help [] in_count
...
let part_one ((rules, updates) : rules_and_updates) : int =
  ...
  *)
  ...
  let get_middle_or_zero update =
    let top =
      topological_sort (List.filter (fun (l, _) -> List.mem l update) rules)
    in
    (*
    Format.printf "Topo: ";
    ...
    *)
    ...
    let iupdate =
      update |> List.map List.assoc_opt
      |> List.map (fun f -> f top_ind)
      |> List.map (Option.value ~default:(-1))
    in
...
let part_two ((rules, updates) : rules_and_updates) : int =
  ...
  let get_middle_or_zero update =
    ...
    if is_increasing (List.hd iupdate) iupdate then
      ...
      update
      |> List.map (fun i ->
             (i, Option.value ~default:(-1) (List.assoc_opt i top_ind)))
...
let parse_file (file_content : string) : rules_and_updates =
  ...
  let empty_line_ind, _ =
    lines |> List.mapi (fun i s -> (i, s)) |> List.find (fun (_, s) -> s = "")
  in
  let rules =
    lines |> List.to_seq |> Seq.take empty_line_ind |> List.of_seq
