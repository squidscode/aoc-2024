...
let is_safe (report : int list) : bool =
  let rec diff (l : int list) (s : int) : int list =
    match l with fst :: rst -> (fst - s) :: diff rst fst | [] -> []
  in
  if List.length report <= 1 then
    true
  else
...
let part_one (reports : int list list) : int =
  List.fold_left
    (fun acc report -> acc + if is_safe report then 1 else 0)
    0 reports
...
let part_two (reports : int list list) : int =
  let rec permutations_one_removed (report : int list) : int list list =
    ...
    | f :: r ->
        let with_first =
          permutations_one_removed r |> List.map (fun l -> f :: l)
        in
        r :: with_first
    | [] -> []
  in
  List.fold_left
    (fun acc report ->
      ...
      if is_safe report then
        1
      else if List.exists is_safe (permutations_one_removed report) then
        1
      else
        0)
    0 reports
