...
type equation = { target : int; values : int list }
...
let can_be_evaluated (equation : equation) : bool =
  ...
  in
  match equation with
  | { target; values } -> help { target; values = List.rev values }
...
let can_be_evaluated_v2 (equation : equation) : bool =
  ...
  *)
  let rec help ?(concat_acc : int = 0) (equation : equation) : bool =
    match equation with
    | { target; values } -> (
        Format.printf "target = %d ; concat_acc = %d ; values = " target
          concat_acc;
        ...
        | x :: xs ->
            ...
            if is_inv_concatable target x then
              help { target = inv_concat target x; values = xs }
            else
              false
        (*
...
let parse_file (file_content : string) : equation list =
  ...
  |> List.map (fun (t, vs) ->
         (t, List.filter (fun s -> String.length s <> 0) vs))
  |> List.map (fun (t, vs) ->
