...
let step (program: program): (program * io) option =
  ...
  let (c, io) = match instruction with
  | Adv combo -> 
    ({ program with a=(
  ...
  | Bxc -> 
    ({program with b = (Int64.logxor program.b program.c)}, None)
  | Out combo -> 
    (program, 
      Some(
        Int64.logand 
          (int_of_combo combo program)
          (Int64.of_int 0b111)
        |> Int64.to_string
        ))
  | Bdv combo -> 
...
let print_program (c : program): unit =
  if c.ip < List.length c.program - 1 then begin
...
let part_one (c : program) : int =
  ...
  printf "\n";
  out
    |> List.fold_left (fun acc r -> acc ^ r) ""
    |> int_of_string
...
let instructions_to_int (instructions : int list) : int =
  instructions 
  |> fun l -> List.fold_right (fun i acc -> 10 * acc + i) l 0
...
let output_compare (o1 : int list) (o2 : int list): int =
  List.fold_right2 (fun i1 i2 acc -> 
    if acc <> 0 then acc else Int.compare i1 i2
  ) o1 o2 0

let rec binary_searchr (oc : (int list -> int list -> int))
  (program : program) (target : int list) (min : int) (max : int): int =
  if max - min = 1 then
    min
  else
...
let rec binary_searchl (oc : (int list -> int list -> int))
  (program : program) (target : int list) (min : int) (max : int): int =
  if max - min = 1 then
    min
  else
...
let parse_file (file_content : string) : program =
  file_content 
  |> Re.all (Re.Perl.re re |> Re.compile)
  ...
  |> fun g -> (
    ...
    let instructions = Re.Group.get g 4
