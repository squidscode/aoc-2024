...
let expand (pmap : pmap) ((score, pos, dir) : score_position) :
  ...
  let a =
    ScorePositionSet.singleton clockwise
    |> ScorePositionSet.add counterclockwise
  in
...
let rec djikstras ?(seen = ScorePositionSet.empty) (pmap : pmap)
    (end_pos : position) (hp : score_position_set) : int =
  ...
  *)
  ...
  let hp =
    ScorePositionSet.remove sp hp
    |> ScorePositionSet.filter (fun (_, pos', dir') ->
           not (pos = pos' && dir = dir'))
  in
  ...
  if pos = end_pos then (* base case / end condition *)
    ...
    ScorePositionSet.find_opt (0, pos, dir) seen |> Option.is_some
  then
    ...
    let new_hp =
      ...
      |> ScorePositionSet.filter (fun (_, pos', dir') ->
             not (pos = pos' && dir = dir'))
      |> ScorePositionSet.filter (fun (_, pos', dir') ->
             seen |> ScorePositionSet.find_opt (0, pos', dir') |> Option.is_none)
    in
...
let path_compare (pth1 : position list) (pth2 : position list) : int =
  ...
  if pthc <> 0 then
    ...
    List.fold_left2
      (fun (acc : int) (pl : position) (pr : position) ->
...
let rec djikstras2 ?(seen : pd_score_path = PDMap.empty) (pmap : pmap)
  ...
  printf "processing: (%d, (%d,%d), %s)\n" score pos.x pos.y
    (string_of_direction dir);
  let hp =
    SPDPSet.remove sp hp
    |> SPDPSet.filter (fun (score', pos', dir', _) ->
           not (pos = pos' && dir = dir' && score < score'))
  in

  if pos = end_pos then (* base case / end condition *)
    hp
    |> SPDPSet.filter (fun (score', pos', _, _) -> score = score' && pos = pos')
    |> SPDPSet.to_list
    |> List.map (fun (s, _, _, p) -> (s, p))
    |> fun l -> (score, path) :: l
  else if
    (* skip expansion if position is already in seen *)
    PDMap.find_opt (pos, dir) seen |> fun opt ->
    match opt with
    | None -> false
    | Some spl ->
        spl
        |> List.fold_left
             (fun b (score', _) ->
               b
               || score'
                  < score (* skip if the score of any seen path is lower *))
             false
  then
    djikstras2 ~seen pmap end_pos hp
  else (* expand all reachable from hp *)
    let new_seen =
      PDMap.find_opt (pos, dir) seen |> Option.value ~default:[] |> fun l ->
      PDMap.add (pos, dir) ((score, path) :: l) seen
    in
    let reachable = expand pmap (score, pos, dir) in
    let new_hp =
      reachable |> ScorePositionSet.to_list
      |> List.map (fun (score, position, direction) ->
             (score, position, direction, position :: path))
      |> SPDPSet.of_list |> SPDPSet.union hp
      |> SPDPSet.filter (fun (score', pos', dir', _) ->
             not (pos = pos' && dir = dir' && score < score'))
    in
...
let part_two (pmap : pmap) : int =
  ...
  in
  print_pmap pmap;
  let ans : (int * position list) list =
    (0, start_pos, Right, [ start_pos ])
    |> SPDPSet.singleton |> djikstras2 pmap end_pos
  in
  List.iter
    (fun (score, path) ->
      printf "SCORE: %d achieved via " score;
      List.iter
        (fun ({ x; y } : position) -> printf "(%d,%d) -> " x y)
        (List.rev path);
      printf "\n")
    ans;
  ans
  |> List.map (fun (_, p) -> p)
  |> List.flatten |> PositionSet.of_list |> PositionSet.cardinal
